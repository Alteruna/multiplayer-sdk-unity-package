...
namespace Alteruna
{
	public class RoomBrowser : BaseRoomBrowser
	{
		...
		{
			if (_selectedRow == null || _selectedRow.Room.SessionID == Multiplayer.CurrentRoom?.SessionID) return;
		...
		private void RoomsUpdated(RoomListUpdatedEvent args)
		{
			List<Room> newRooms = args.Controller.AvailableRooms;
		...
		private void Connected(ConnectedEvent args)
		{
			if (args.Controller.InRoom)
			{
				JoinedRoom(args.Controller.CurrentRoom);
			}
		}
		...
		private void JoinedRoom(RoomJoinedEvent args) => JoinedRoom(args.Room);
		private void JoinedRoom(Room room)
		{
			...
			{
				CustomRoomInfo roomInfo = Reader.DeserializePackedString<CustomRoomInfo>(room.CustomData);
				Multiplayer.LoadScene(roomInfo.SceneIndex, SpawnAvatarAfterLoad);
			}
		}

		private void JoinRejected(JoinRejectedEvent args)
		{
			string message = args.Reason == "SessionInvalidPin" ? "Incorrect password" : args.Reason;
		...
		private List<RoomBrowserRow> FilterRows()
		{
			...
			return _activeRows.Where(r =>
				r.RoomName.ToUpper().Contains(searchText) // If searchText contains text, only show rooms with names that contain the text;
				&& !(hideLockedRooms && r.Room.IsPasswordProtected) // If hideLockedRooms, hide rooms that requires passwords to join;
				&& !(hideFullRooms && r.Room.CurrentUsers >= r.Room.MaxUsers) // If hideFullRooms, hide rooms where connected users >= MaxUsers;
			).ToList();
		}
